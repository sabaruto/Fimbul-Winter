...
namespace Base_Classes
{
  public class Character : MonoBehaviour
  {
    ...
    {
      foreach (Ability ability in abilityList) ability.Terminate();
    ...
    public bool IsAlive() { return alive; } //isAlive

    public bool HasAbility() { return usesAbility; } //hasAbility
    ...
    public Ability[] GetAbilityList() { return abilityList; }

    // Finding out if the enemy is ranged or not depending on the size of the 
    // range
    public virtual bool IsRanged() { return projectile != null; }

    public bool IsStunned() { return isStunned; }

    public void SetStunned(bool requiredStunned) { isStunned = requiredStunned; }
    ...
    private void MoveToAttack()
    {
      ...
      // wether the character can see the target
    ...
    public void AddAttackDamage(int damage) { basicDamage += damage; }


    // Gets the casting boolean
    public bool GetCasting() { return isCasting; }
    ...
    public void SetAttackStatus(Status status) { projStatus = status; }
    ...
    private void Attack()
    {
      ...
      if (IsRanged())
      {
        ...
        // target
        GameObject aProjectile = Instantiate(projectile, transform.position,
          Quaternion.identity);

        BasicAttackProjectile projectileAttack =
