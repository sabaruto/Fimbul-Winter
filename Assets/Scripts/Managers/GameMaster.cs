...
namespace Managers
{
  public class GameMaster : MonoBehaviour
  {
    ...
    private void Start()
    {
      ...
      foreach (Character scr in charScript) characters.Add(scr.gameObject);
      ...
      foreach (Helper scr in helpScript) clickableObjects.Add(scr.gameObject);
      ...
      foreach (GameObject obst in obstacles) SetObstacle(obst);
    ...
    private void Update()
    {
      ...
      {
        Character currentCharacter = character.GetComponent<Character>();
        if (currentCharacter.HasAbility())
          foreach (Ability ability in currentCharacter.GetAbilityList())
    ...
    public T CheckPerson<T>(Vector2 position) where T : MonoBehaviour
    {
      ...
      foreach (GameObject character in clickableObjects)
      {
        T currentPerson = character.GetComponent<T>();
    ...
    public void SetObstaclePoint(int x, int y)
    {
      ...
      foreach (GameObject character in characters) character.GetComponent<Movement>().GetGrid().SetObstacle(x, y);
    }
    ...
    {
      // Looping through all the characters
      foreach (GameObject character in characters) character.GetComponent<Movement>().GetGrid().SetObstacle(position);
    }
    ...
    private void SetObstacle(GameObject obstacle)
    {
      ...
      Collider2D currentCollider = obstacle.GetComponent<Collider2D>();
      ...
      for (var xIndex = centre.x - xExtends; xIndex <= centre.x + xExtends;
        xIndex += gridResolution)
      {
        for (var yIndex = centre.y - yExtends;
          yIndex <= centre.y + yExtends; yIndex += gridResolution)
        {
          // The obstacle point
          Vector2 obstaclePoint = new Vector2(xIndex, yIndex);
