...
namespace Enemies
{
  public class Svida : Enemy
  {
    ...
    private new void Awake()
    {
      ...
      // he goes inside
      Ability rainOfFire = new Ability("Rain Of Fire", 2, 5, 0, 2f,
        StartRainOfFire, 0.4f);

      // Svida makes a geyser of lava which erupts from the ground, damaging 
      // and burning the player
      Ability burst = new Ability("Burst", 2, 5, 0, 2f, StartBurst, 0.3f);

      // Svida throws a flaming rock at the player, damaging and burning the
      // player
      Ability flameThrow = new Ability("Flame Throw", 10, 4, 0, 3, FlameThrow, 0.2f);
    ...
    private void StartRainOfFire() { fireTimer = fireResetTime; }
    ...
    private void StartBurst() { lavaTimer = lavaResetTime; }
    ...
    {
      GameObject flameRock = Instantiate(flameRockProjectile, transform.position,
        Quaternion.identity);
