...
public class MyGrid
{
    ...
    {
        return coordinates[0] + coordinates[1] * length;
    }
    ...
    public void SetObstacle(int x, int y)
    {
        ...
        var nodePosition = NodeVector(new[] {x, y});
        ...
        int[] endNodePosition)
    {
        ...
        var endNodeVector = NodeVector(endNodePosition);
    ...
    private List<Node> PruneRoute(List<Node> fullRoute)
    {
        ...
        for (var nodeIndex = 0; nodeIndex < fullRoute.Count; nodeIndex++)
        {
            ...
            var previousDirection = fullRoute[nodeIndex].GetPosition() -
                                    fullRoute[nodeIndex - 1].GetPosition();
            previousDirection.Normalize();

            // Checking the direction of the future node
            var nextDirection = fullRoute[nodeIndex + 1].GetPosition() -
    ...
    public bool LineOfSight(Vector2 startPosition, Vector2 endPosition)
    {
        ...
        var startVector = new Vector2Int(startCoor[0], startCoor[1]);
        var endVector = new Vector2Int(endCoor[0], endCoor[1]);
        ...
        while (Vector2Int.RoundToInt(currentVector) != endVector)
        {
            ...
            var roundVector = Vector2Int.RoundToInt(currentVector);
    ...
    public void SetGridPosiiton(Vector2 pos)
    {
        position = pos;
    }
    ...
    public void OnDrawGizmosSelected()
    {
        ...
        for (var nodeIndex = 0; nodeIndex < grid.Length; nodeIndex++)
        {
            ...
            var nodeVector = NodeVector(Coordinates(nodeIndex));
