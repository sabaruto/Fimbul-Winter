...
namespace Menus
{
  public class SkillMenu : MonoBehaviour
  {
    ...
    [SerializeField] private Button[] volleyButtons,
      piercingArrowButtons, poisonButtons;
    ...
    public void UpdateGui()
    {
      ...
      for (var skillIndex = 0; skillIndex < buttons.Length; skillIndex++)
      {
        ...
        SkillState state = skillStatus[skillName];

        // Checks if the current skill is default
        for (var buttonIndex = 0; buttonIndex < 2; buttonIndex++)
        {
          Button currentButton = buttons[skillIndex][buttonIndex];
    ...
    {
      SkillState newState = SkillState.Default;
      ...
      SkillState status = skillStatus[skillName];
    ...
    public void UpgradeSkillLeft(string skillName) { UpgradeSkill(skillName, 0); }

    public void UpgradeSkillMiddle(string skillName) { UpgradeSkill(skillName, 1); }

    public void UpgradeSkillRight(string skillName) { UpgradeSkill(skillName, 2); }
  }
}