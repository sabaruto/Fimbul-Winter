...
namespace ScriptCalculator.Editor
{
    public class ScriptCalculator : EditorWindow
    {
        ...
        private bool _searchInDirectory;
        private bool _scriptExtensionsCS = true;
        private bool _scriptExtensionsJS = true;
        private bool _passBlankLines;
        private bool _isActiveFilesArea;
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Search", GUILayout.Width(290)))
            {
                ...
                var _assetCount = AssetDatabase.FindAssets("t:Script").Length;

                for (var i = 0; i < _assetCount; i++)
                {
                    var _progreassValue = 1f / (float)_assetCount;
                    EditorUtility.DisplayProgressBar("Please Wait", "Calculating...", _progreassValue);

                    var _file = AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets("t:Script")[i]);
